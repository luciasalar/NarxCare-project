...
```{r binary, echo=FALSE}
set.seed(100)
#X_7 Serious Adverse Event, these events are weakly correlated with hospital visits
#var1, pho, threshold, seed, location_var2, shape_var2, threshold_2

#X_7:Serious Adverse Event -fall
...
#X_7:Serious Adverse Event - Acetaminophen
...
#X_7:Serious Adverse Event - Poisoning by Sedative
error=rnorm(number_of_cases, 2, 10)
...
#X_7: Serious Adverse Event - Vehicular Accidents
error=rnorm(number_of_cases, 2, 10)
...
#X_7:Serious Adverse Event - Other Accidents, no correlation with other vars m = 0.002
...
#X_7:Serious Adverse Event - Poisoning by Other  m = 0.0008
error=rnorm(number_of_cases, 2, 10)
...
#X8: insomnia medication, Barbiturate, Sedative Use Disorder, binary
...
```{r storm, echo=FALSE}
set.seed(100)
#X25 MME  Morphine equivalent daily dose 10 months ago, continous ,  these variables are accumulative 
...
#Opioids For Pains in the past 182 to 365 days binary  cor[0.62, 0.21] m, sd = [0.12, 0.33]
...
#no corr with other var
Dx_Endometriosis_0to1_Y <- rbinom(number_of_cases, 1,.0006)
summary(Dx_Endometriosis_0to1_Y)


#X24 mental illnesses
#psychoses past year, this one is related to age (-0.14) and substance use such as alcohol(0.11) cannabis, cocaine, nicotine, depression
...
#depression, correlated with alcoho, psychosis, homelessness, elixhausr drug abuse
error=rnorm(number_of_cases, 0, 5)
...
#X:26 homelessness [0.16, 0.10, 0.16, 0.15, 15] m = 0.02
error=rnorm(number_of_cases, 0, 5)
...
xb <- -400 +  1*Ux_OP_PoC_ED + 10*Ux_OP_PoC_UC + 30*Ux_OP_PoC_TobaccoCessation  + 3*Ux_OP_MHOC_HBPC +   3.5*TotalNumOfAppt + 195*MOPROB_AllAppt   + 3*Dx_SAE_Acetaminophen_0to1_Y  + 30*Dx_SAE_Sedative_0to1_Y + 60*Dx_SAE_Vehicle_0to1_Y  + 20*Dx_SAE_OtherAccident_0to1_Y + 30*Dx_SAE_OtherDrug_0to1_Y  + 20*Dx_SAE_Falls_0to1_Y  +   3*Rx_Insomnia_0to365_D + 35*Paton_Rx_Insomnia  + Rx_OAT_methadone_12m + Rx_OAT_buprenorphine_12m + 0.5*Rx_Anxiolytic_0to365_D +  1*Dx_SUD_CatchAll_RV2_0to1_Y + 3*Dx_SedativeUD_0to1_Y + 1*Dx_Amphetamine_OtherStim_UD_0to1 + 1*Dx_AlcoholUD_0to1_Y + 0.004*agegroup_ls30 + 28*agegroup_ge30 + 28*agegroup_ge40 + 28*agegroup_ge50 + 28*agegroup_ge60 - 30*agegroup_ge70 - 25*sex_female + 43*race_White  + 20*Dx_UDS1_12m + 20*Dx_UDS2_12m+   Dx_UDS3_12m + Ux_RehMedPain + 35*Dx_Pain_Neuropathy_0to1_Y  + 35*Dx_Pain_Back + 35*Dx_Pain_Neck_0to1_Y + 1*Dx_Pain_Urogenital_0to1_Y + 1*Dx_Pain_Fibromyalgia_0to1_Y  +  20*Dx_Endometriosis_0to1_Y + 20*Dx_MDD_12m+ 5*EH_Psychoses + 0.5*Dx_PTSD_1to2_Y  + 0.005*Rx_Medd + 0.005*Rx_Medd_10m + 0.003*Rx_Medd_11m + 0.001*Rx_Medd_1m +0.001*Rx_Medd_2m + 0.001*Rx_Medd_3m + 0.001*Rx_Medd_4m + 0.001*Rx_Medd_5m + 0.003*Rx_Medd_6m + 0.003*Rx_Medd_7m + 0.005*Rx_Medd_8m + 0.004*Rx_Medd_9m + 0.1*Dx_Homeless_1to2_Y +  0.5*Dx_EH_Obesity_1to2_Y  + 5*Rx_OpioidForPain_0to182_D + 10*Rx_OpioidForPain_182to365_D + error
...
transform_mean <- function(percent, var){
  ...
  N1 <- sum(var != 0) 
  #new value = mean * percent that we want to reduce
  reduce  <- mean(var)*percent
  print(reduce)
  #normalize the variable to be applid
  reduce_scaled <- reduce*(length(var)/N1)
  print(reduce_scaled)
  #apply reduction function on normal
