...
#This is the logistics regression classifier with a weighted cost function, the weights are defined in experiment_weighted.yaml
...
class PreprocessData:
	...
	def get_cohorts(self):
		...
		'''
		data = pd.read_csv(self.data_path + "simulated_data_big_sample_reduced_mean10.csv")
        
        #select class 1 and class 0
		...
		#combine all the positive cases with all the negative cases
	...
	def pre_process(self): 
		...
		"""
		data = self.get_cohorts()  #withdraw one cohort from the population

		X = data.drop(columns=[self.label]) #feature matrix, drop outcome
		y = data[self.label] #outcome variable
...
class ColumnSelector:
	...
	def transform(self, X):
		#here we select the feature columns (columns are defined in the yaml file)
...
class Training:
	...
	def setup_pipeline(self, classifier):
		...
		('other_features', Pipeline([
				...
				('selector', ColumnSelector(columns=features_col)),  #select features
...
i = 0
while i < 500: #set the number of times we run the experiments
	...
	for classifier in experiment['experiment']:
		...
		for feature_key, features_list in experiment['features'].items():
			...
			TPR_backpain, FPR_backpain, PPV_backpain, TPR_nonBackpain, FPR_nonBackpain, PPV_nonBackpain= train.get_group_evaluation('PA_Pain_Back') #back pain
			TPR_neckpain, FPR_neckpain, PPV_neckpain, TPR_nonNeckpain, FPR_nonNeckpain, PPV_nonNeckpain= train.get_group_evaluation('PA_Pain_Neck_0to1_Y') #neck pain
			TPR_Neuropathy, FPR_Neuropathy, PPV_Neuropathy, TPR_nonNeuropathy, FPR_nonNeuropathy, PPV_nonNeuropathy = train.get_group_evaluation('PA_Pain_Neuropathy_0to1_Y') #neurological pain
			TPR_Fibromyalgia, FPR_Fibromyalgia, PPV_Fibromyalgia, TPR_noFibromyalgia, FPR_noFibromyalgia, PPV_noFibromyalgia= train.get_group_evaluation('PA_Pain_Fibromyalgia_0to1_Y') #Fibromyalgia
			...
			TPR_PTSD, FPR_PTSD, PPV_PTSD, TPR_noPTSD, FPR_noPTSD, PPV_noPTSD= train.get_group_evaluation('MH_PTSD_1to2_Y') #PTSD
			TPR_MDD, FPR_MDD, PPV_MDD, TPR_noMDD, FPR_noMDD, PPV_noMDD= train.get_group_evaluation('MH_MDD_12m') #MDD
			TPR_Homeless, FPR_Homeless, PPV_Homeless, TPR_noHomeless, FPR_noHomeless, PPV_noHomeless= train.get_group_evaluation('EH_Homeless_1to2_Y') #Homelessness
			...
			result_row = [[grid_search.best_score_, grid_search.best_params_, 'weighted', report['1']['precision'], report['1']['recall'], report['1']['f1-score'], report['accuracy'], TPR, FPR, PPV, log_loss, str(datetime.datetime.now()), classifier, feature_key,  TPR_nonWhite, TPR_White, TPR_backpain, TPR_nonBackpain, TPR_neckpain, TPR_nonNeckpain, TPR_Neuropathy, TPR_nonNeuropathy, TPR_Fibromyalgia, TPR_noFibromyalgia, TPR_PTSD, TPR_noPTSD, TPR_MDD, TPR_noMDD, TPR_Homeless, TPR_noHomeless, FPR_nonWhite, FPR_White, FPR_backpain, FPR_nonBackpain, FPR_neckpain, FPR_nonNeckpain, FPR_Neuropathy, FPR_nonNeuropathy, FPR_Fibromyalgia, FPR_noFibromyalgia,  FPR_PTSD, FPR_noPTSD, FPR_MDD, FPR_noMDD, FPR_Homeless, FPR_noHomeless, PPV_nonWhite, PPV_White, PPV_backpain, PPV_nonBackpain, PPV_neckpain, PPV_nonNeckpain, PPV_Neuropathy, PPV_nonNeuropathy, PPV_Fibromyalgia, PPV_noFibromyalgia, PPV_PTSD, PPV_noPTSD, PPV_MDD, PPV_noMDD, PPV_Homeless, PPV_noHomeless,10]]
